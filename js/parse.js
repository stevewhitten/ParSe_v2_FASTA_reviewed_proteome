const amino_acids = {
    A: { name: 'ala', ppii_h: 0.37, helix: 1.42, hydr: 0.0728, lambda: 0.0, },
    C: { name: 'cys', ppii_h: 0.25, helix: 0.73, hydr: 0.3557, lambda: 0.0, },
    D: { name: 'asp', ppii_h: 0.3, helix: 1.01, hydr: -0.0552, lambda: -1.0, },
    E: { name: 'glu', ppii_h: 0.42, helix: 1.63, hydr: -0.0295, lambda: -1.0, },
    F: { name: 'phe', ppii_h: 0.17, helix: 1.16, hydr: 0.4201, lambda: 0.0, },
    G: { name: 'gly', ppii_h: 0.13, helix: 0.50, hydr: -0.0589, lambda: 0.0, },
    H: { name: 'his', ppii_h: 0.2, helix: 1.20, hydr: 0.0874, lambda: 0.0, },
    I: { name: 'ile', ppii_h: 0.39, helix: 1.12, hydr: 0.3805, lambda: 0.0, },
    K: { name: 'lys', ppii_h: 0.56, helix: 1.24, hydr: -0.0053, lambda: 1.0, },
    L: { name: 'leu', ppii_h: 0.24, helix: 1.29, hydr: 0.3819, lambda: 0.0, },
    M: { name: 'met', ppii_h: 0.36, helix: 1.21, hydr: 0.1613, lambda: 0.0, },
    N: { name: 'asn', ppii_h: 0.27, helix: 0.71, hydr: -0.0390, lambda: 0.0, },
    P: { name: 'pro', ppii_h: 1.0, helix: 0.65, hydr: -0.0492, lambda: 0.0, },
    Q: { name: 'gln', ppii_h: 0.53, helix: 1.02, hydr: 0.0126, lambda: 0.0, },
    R: { name: 'arg', ppii_h: 0.38, helix: 1.06, hydr: 0.0394, lambda: 1.0, },
    S: { name: 'ser', ppii_h: 0.24, helix: 0.71, hydr: -0.0282, lambda: 0.0, },
    T: { name: 'thr', ppii_h: 0.32, helix: 0.78, hydr: 0.0239, lambda: 0.0, },
...
function getResidues(input, windowSize) {
    var residues = [];
    ...
    for (let i = 0; i < input.length; i++) {
        if (i < input.length - windowSize + 1) {
            var middle_position = i + Math.trunc(windowSize / 2);
            var count = {
                A: 0,
                C: 0,
                D: 0,
            ...
            for (let j = i; j < i + windowSize; j++) {
                if (amino_acid_keys.includes(input[j])) {
                    count[input[j]]++;
                }
            }
            ...
            ppii = ppii / windowSize;
            helix = helix / windowSize;
            hydr = hydr / windowSize;
            ...
            if (hydr >= 0.08280152) {
                ...
                var U_q = 0;
                var scd = 0;
/* this section commented out to speed up the calculation, re-enable if U_pi and U_q are needed
                ...
                for (let j = i; j < i + windowSize; j++) {
                    for (let k = i; k < i + windowSize; k++) {
                        if (k > j) {
                            ...
                            if (amino_acid_keys.includes(input[j])) {
                                var lambda_1 = amino_acids[input[j]].lambda;
                            }
                            if (amino_acid_keys.includes(input[k])) {
                                var lambda_2 = amino_acids[input[k]].lambda;
                            }
...
*/
                ...
                var rh = 2.16 * (4 * windowSize) ** v_exponent + 0.26 * 4 * net_charge - 0.29 * (4 * windowSize) ** 0.5;
                var nu_model = Math.log(rh / 2.16) / Math.log(4 * windowSize);
...
function getRegions(residues, regionLength, cutoff) {
    // Using separate region arrays for special file parsing code
    var regions = [];
    var regionList;
...
function parse(sequence, windowSize, regionLength, cutoff) {
    var result = {
        error: false,
        message: null,
        data: null,
    ...
    var residues = getResidues(sequence, windowSize);
